...
import { useFiltersContext, useFiltersPresetsProvider } from '@rovna-internal/filters/contexts';
...
const Item = ({ preset, onClick, onRemove, onEdit, INTERNAL_scope }: ItemProps) => {
  ...
  const checked = React.useMemo(
    () => isEqual(pickBy(values, identity), pickBy(preset.value, identity)),
    [preset.value, values],
  );

  useKeyPress('Enter', () => {
    const rollback = preset.label;
    const next = value.trim();

    const isSame = next === rollback;
    const isEmpty = next === '';

    if (!editing || isSame || isEmpty) {
      setEditing(false);
      setValue(rollback);
